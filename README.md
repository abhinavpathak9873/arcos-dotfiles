...
The complete ArcOS v1.3.2 SwayFX rice: reproducible NixOS modules, native desktop
surfaces, wallpaper-derived theme templates, scripts, terminal tooling, and
ordinary user-facing dotfiles.

![ArcOS v1.3.2 desktop](media/25-v13-desktop.png)
...
| `Meta+A` | Centered categorized application launcher |
| `Meta+W` | Centered overview of occupied workspaces and their applications |
...
| `Meta+N` | Top-right Control Center and notifications |
...
| Application launcher | Workspace overview |
| --- | --- |
| ![Application launcher](media/33-v13-application-launcher.png) | ![Occupied-workspace overview](media/27-v13-overview.png) |

| Control Center | Live local dictation |
| --- | --- |
| ![Control Center](media/29-v13-quick-controls.png) | ![Voxtype dictation waveform](media/34-v13-voxtype-dictation.png) |

![Exact ArcOS 1.3.1 ISO boot and installer](media/35-v131-exact-iso-installer.png)

![Exact compact ArcOS 1.3.2 ISO boot and installer](media/36-v132-compact-iso-installer.png)

The desktop surfaces are generated by the ArcOS graphical NixOS/KVM test. The
Voxtype frame is captured while real PipeWire audio is decoded locally. The
installer frame comes from the exact release ISO after an OVMF/Q35 boot with a
writable virtual target disk.

The v1.3.2 hybrid image is `6,961,360,896` bytes (`6.48 GiB`). Its compact
profile keeps the complete desktop, universal graphics drivers, Voxtype,
OrcaSlicer, Steam, OBS, Chrome, VS Code, and LibreOffice offline-ready. Dormant
Hermes/Kokoro payloads, Cura as the second slicer, OBS browser sources, and
GE-Proton are installed on demand instead of being duplicated on every USB.
